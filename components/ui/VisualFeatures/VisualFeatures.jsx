import SectionWrapper from "@/components/SectionWrapper"
import Feature1 from "@/public/images/Feature-1.svg"
import Feature2 from "@/public/images/Feature-2.svg"
import Image from "next/image"
import { motion } from "framer-motion";

const VisualFeatures = () => {

    const features = [
        {
            title: "Hassle Free",
            desc: "We ensure that you enjoy a seamless and smooth experience.",
            img: Feature1
        },
        {
            title: "A Reasonable price for Premium Software ",
            desc: "Tendertulip Software is the perfect solution for busy professionals who need to quickly create or experiment engaging applications.",
            img: Feature2
        },
    ]
    return (
        <SectionWrapper>
            <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1, transition: { duration: 1 } }} className="custom-screen text-gray-300">
                <div className="max-w-xl mx-auto text-center">
                    <h2 className="text-gray-50 text-3xl font-semibold sm:text-4xl">
                        Take your Business to the next level with <span className="text-purple-500 underline underline-offset-4 decoration-dashed">Tendertulip Software</span>
                    </h2>
                    <p className="mt-3">
                        With Mailgo's powerful features, you can easily create and send beautiful emails that will engage your customers and drive more sales.
                    </p>
                </div>
                <div className="mt-12">
                    <ul className="space-y-8 gap-x-6 sm:flex sm:space-y-0">
                        {
                            features.map((item, idx) => (
                                <li className="flex-1 flex flex-col justify-between border border-purple-700 border-opacity-70 rounded-2xl" key={idx}
                                    style={{
                                        background: "radial-gradient(141.61% 141.61% at 29.14% -11.49%, rgba(203, 213, 225, 0.15) 0%, rgba(203, 213, 225, 0) 57.72%)"
                                    }}
                                >
                                    <div className="p-8">
                                        <h3 className="text-gray-50 text-xl font-semibold">
                                            {item.title}
                                        </h3>
                                        <p className="mt-3 sm:text-sm md:text-base">
                                            {item.desc}
                                        </p>
                                    </div>
                                    <div className="pl-9">
                                        <Image
                                            src={item.img}
                                            className="w-full ml-auto"
                                            alt={item.title}
                                        />
                                    </div>
                                </li>
                            ))
                        }
                    </ul>
                </div>
            </motion.div>
        </SectionWrapper>
    )
}

export default VisualFeatures